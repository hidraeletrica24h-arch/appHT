              ...
              <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-1">
                    ...
                    <input defaultValue={config.companyName || "HidraElétrica PRO"} onChange={(e) => updateConfig('companyName', e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 text-white" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-zinc-500 uppercase">CNPJ</label>
                    <input defaultValue={config.cnpj || "00.000.000/0001-00"} onChange={(e) => updateConfig('cnpj', e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 text-white" />
                  </div>
                </div>
              </div>
              <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-2xl space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">Verifica a integração com o Supabase.</p>
                  </div>
                  <button onClick={handleCheckSupabase} disabled={isCheckingSupabase} className="px-4 py-2 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-500 rounded-lg text-sm font-bold transition-colors disabled:opacity-50">
                ...
                {supabaseStatus && (
                  <div className={`p-4 rounded-xl text-sm font-medium ${supabaseStatus.success ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'}`}>
                    {supabaseStatus.success ? 'Conexão estabelecida com sucesso!' : `Erro: ${supabaseStatus.error}`}
                  </div>
                )}
              </div>
            </div>
